      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          <div class="header-left">
            ...
            <span class="header-title">{{ $t('graph.realtimeTitle') }}</span>
          </div>
          <div class="header-right">
            <template v-if="graphData">
              <span class="stat-item">{{ graphData.node_count || graphData.nodes?.length || 0 }} {{ $t('graph.nodes') }}</span>
              <span class="stat-divider">|</span>
              <span class="stat-item">{{ graphData.edge_count || graphData.edges?.length || 0 }} {{ $t('graph.edges') }}</span>
              <span class="stat-divider">|</span>
            </template>
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="graphLoading" class="graph-loading">
            ...
            <p class="loading-text">{{ $t('graphPanel.loadingGraphData') }}</p>
          </div>
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">{{ $t('process.ontologyCompleteHint') }}</p>
          </div>
          
          <!-- buildinbutstill没有data -->
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  {{ $t('process.phases.ontology.description') }}
                </div>
              </div>
              
              <!-- ontologygenerateprogress -->
              ...
              <!-- alreadygenerate的ontologyinformation -->
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">{{ $t('process.apiDescription') }}</div>
                <div class="detail-content">
                  {{ $t('process.phases.graph.description') }}
                </div>
              </div>
              
              <!-- waitontologycomplete -->
              ...
              <div class="detail-section" v-if="graphData">
                ...
                <div class="build-result">
                  <div class="result-item">
                    ...
                    <span class="result-label">{{ $t('process.results.entityNodes') }}</span>
                  </div>
                  <div class="result-item">
                    <span class="result-value">{{ graphData.edge_count }}</span>
                    <span class="result-label">{{ $t('process.results.relationEdges') }}</span>
                  </div>
                  <div class="result-item">
                    <span class="result-value">{{ entityTypes.length }}</span>
                    <span class="result-label">{{ $t('process.results.entityTypes') }}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                ...
                <div class="phase-api">{{ $t('process.phases.complete.description') }}</div>
              </div>
...
// currentProject ID（can能from'new'变为实际ID）
...
const ontologyProgress = ref(null) // ontologygenerateprogress
const currentPhase = ref(-1) // -1: on传in, 0: ontologygeneratein, 1: graphbuild, 2: complete
const selectedItem = ref(null) // 选in的nodeoredge
...
const statusText = computed(() => {
  ...
  if (currentPhase.value >= 2) return t('status.buildCompleted')
...
// initialize - processnew建projectorloadalready有project
const initProject = async () => {
  const paramProjectId = route.params.projectId
  
  if (paramProjectId === 'new') {
    // new建project：from store get待on传的data
...
// processnew建project - call ontology/generate API
const handleNewProject = async () => {
  ...
  try {
    loading.value = true
    currentPhase.value = 0 // ontologygenerate阶segment
    ontologyProgress.value = { message: t('process.messages.uploadingAndAnalyzing') }
    ...
    if (response.success) {
      ...
      error.value = response.error || t('process.errors.ontologyGenerationFailed')
    }
  } catch (err) {
    console.error('Handle new project error:', err)
    error.value = t('process.errors.projectInitFailed') + ': ' + (err.message || t('common.unknown'))
  } finally {
    loading.value = false
  }
}

// loadalready有projectdata
const loadProject = async () => {
  try {
    ...
    error.value = t('process.errors.loadProjectFailed') + ': ' + (err.message || t('common.unknown'))
  } finally {
    loading.value = false
  }
}
...
const updatePhaseByStatus = (status) => {
  switch (status) {
    ...
    case 'graph_completed':
      ...
      break
    case 'failed':
      error.value = projectData.value?.error || t('process.errors.processFailed')
      break
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    buildProgress.value = {
      progress: 0,
      message: t('process.messages.startingGraphBuild')
    }
    
    const response = await buildGraph({ project_id: currentProjectId.value })
    
    if (response.success) {
      buildProgress.value.message = t('process.messages.graphBuildTaskStarted')
      
      // save task_id for轮询
      const taskId = response.data.task_id
      
      // StartGraph data轮询（独立于taskstatus轮询）
      startGraphPolling()
      
      // Starttaskstatus轮询
      startPollingTask(taskId)
    } else {
      error.value = response.error || t('process.errors.startGraphBuildFailed')
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = t('process.errors.startGraphBuildFailed') + ': ' + (err.message || t('common.unknown'))
    buildProgress.value = null
  }
}
...
const fetchGraphData = async () => {
  try {
    // firstgetprojectinformation以get graph_id
  ...
  try {
    ...
    if (response.success) {
      ...
      buildProgress.value = {
        ...
        message: task.message || t('process.messages.processing')
      }
      
      console.log('Task status:', task.status, 'Progress:', task.progress)
      
      if (task.status === 'completed') {
        console.log('✅ ' + t('process.messages.graphBuildCompleteLoading'))
        ...
        buildProgress.value = {
          progress: 100,
          message: t('process.messages.buildCompleteLoadingGraph')
        }
        
        // 重newLoad projectdataget graph_id
        const projectResponse = await getProject(currentProjectId.value)
        if (projectResponse.success) {
          projectData.value = projectResponse.data
          
          // finalloadcompleteGraph data
          if (projectResponse.data.graph_id) {
            console.log('📊 ' + t('process.messages.loadingCompleteGraph') + ':', projectResponse.data.graph_id)
            await loadGraph(projectResponse.data.graph_id)
            console.log('✅ ' + t('process.messages.graphLoadComplete'))
          }
        }
        ...
        stopPolling()
        stopGraphPolling()
        error.value = t('process.errors.graphBuildFailed') + ': ' + (task.error || t('common.unknown'))
        buildProgress.value = null
      }
    }
  } catch (err) {
...
const renderGraph = () => {
  ...
  if (nodesData.length === 0) {
    ...
    svg.append('text')
      ...
      .attr('fill', '#999')
      .text(t('graph.waitingForData'))
    return
  }
  
  // createnodemappingforfindname
  const nodeMap = {}
  nodesData.forEach(n => {
    nodeMap[n.uuid] = n
  })
  ...
  // createnodeID集合forfiltervalidedge
  ...
  // 点击whitespace处closedetails面板
  svg.on('click', () => {
    closeDetailPanel()
  })
  
  simulation.on('tick', () => {
    // updatealledge的位置（包括can见线and透明点击区域）
