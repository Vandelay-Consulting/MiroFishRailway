<template>
  ...
  >
    <!-- Background decoration: technical grid lines (only show when there are projects) -->
    ...
    <!-- Card container (only show when there are projects) -->
        ...
        <div class="card-header">
          ...
          <div class="card-status-icons">
            <span
              ...
              :title="$t('history.steps.graphBuild')"
            >◇</span>
            <span
              class="status-icon available"
              :title="$t('history.steps.envSetup')"
            >◈</span>
            <span
              class="status-icon"
              :class="{ available: project.report_id, unavailable: !project.report_id }"
              :title="$t('history.steps.report')"
            >◆</span>
          </div>
        </div>
        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-actions">
              ...
              >
                <span class="btn-step">{{ $t('steps.step') }}1</span>
                <span class="btn-icon">◇</span>
                <span class="btn-text">{{ $t('history.steps.graphBuild') }}</span>
              </button>
              <button
                class="modal-btn btn-simulation"
                @click="goToSimulation"
              >
                <span class="btn-step">{{ $t('steps.step') }}2</span>
                <span class="btn-icon">◈</span>
                <span class="btn-text">{{ $t('history.steps.envSetup') }}</span>
              </button>
              <button
              ...
              >
                <span class="btn-step">{{ $t('steps.step') }}4</span>
                <span class="btn-icon">◆</span>
                <span class="btn-text">{{ $t('history.steps.report') }}</span>
              </button>
            </div>
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate current row card count, ensure centering per row
    ...
    const x = offset * 35
    // Adjust start position, close to title but keep proper spacing
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // If target state same as current state, no need to handle
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Release lock after animation completes, check for pending state changes
          setTimeout(() => {
            isAnimating = false

            // After animation ends, check for new pending state
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay execution a bit to avoid switching too fast
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Adjust rootMargin, shrink viewport bottom, need to scroll more to trigger expand
