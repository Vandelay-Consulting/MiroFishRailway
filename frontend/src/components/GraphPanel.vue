    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? $t('graphPanel.graphRagMemoryUpdating') : $t('graphPanel.realtimeUpdating') }}
        </div>
      ...
      <div v-else-if="loading" class="graph-state">
        ...
        <p>{{ $t('graphPanel.loadingGraphData') }}</p>
      </div>
...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Transitioned from simulating to non-simulating, show completion hint
...
const renderGraph = () => {
  ...
  tempEdges.forEach(e => {
    ...
    if (totalCount > 1) {
      ...
      // This ensures all edges are distributed in the same reference frame regardless of direction
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add gravitational force toward center to gather isolated node clusters
  ...
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const y2 = sy + 4
      // Use arc to draw self-loop (sweep-flag=1 for clockwise)
    ...
    // Perpendicular offset to line direction, calculated as proportion of distance to ensure visible curve
    // More edges = larger proportion of distance as offset
  ...
  const getLinkMidpoint = (d) => {
    ...
    // Quadratic Bezier formula B(t) = (1-t)²P0 + 2(1-t)tP1 + t²P2, at t=0.5
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect if real dragging started (movement exceeds threshold)
