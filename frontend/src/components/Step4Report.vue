  ...
  <div class="report-panel">
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          <div class="workflow-metrics">
            <div class="metric">
              <span class="metric-label">{{ $t('steps.report.sections') }}</span>
              <span class="metric-value mono">{{ completedSections }}/{{ totalSections }}</span>
            </div>
            <div class="metric">
              <span class="metric-label">{{ $t('steps.report.elapsed') }}</span>
              <span class="metric-value mono">{{ formatElapsedTime }}</span>
            </div>
            <div class="metric">
              <span class="metric-label">{{ $t('steps.report.toolsLabel') }}</span>
        ...
        <div class="workflow-timeline">
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <template v-if="log.action === 'llm_response'">
                    <div class="llm-meta">
                      <span class="meta-tag">{{ $t('steps.report.iteration') }} {{ log.details?.iteration }}</span>
                    ...
                    <div v-if="log.details?.has_final_answer" class="final-answer-hint">
                      ...
                      <span>{{ $t('steps.report.sections') }} "{{ log.section_title }}" {{ $t('steps.report.contentGenerated') }}</span>
                    </div>
                  ...
                  <template v-if="log.action === 'report_complete'">
                    <div class="complete-banner">
                      ...
                      <span>{{ $t('steps.report.complete') }}</span>
                    </div>
                  </template>
                </div>
          ...
          <div v-if="agentLogs.length === 0 && !isComplete" class="workflow-empty">
            ...
            <span>{{ $t('steps.report.waitingActivity') }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="console-logs">
      <div class="log-header">
          <span class="log-title">{{ $t('steps.report.consoleOutput') }}</span>
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          ...
          if (interview.twitterAnswer && interview.twitterAnswer !== '（该platformnot获得回复）') {
