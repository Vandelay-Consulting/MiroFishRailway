  ...
  <div class="simulation-panel">
    ...
    <div class="control-bar">
      <div class="status-group">
        ...
        <div class="platform-status twitter" :class="{ active: runStatus.twitter_running, completed: runStatus.twitter_completed }">
          <div class="platform-header">
            ...
            <span class="platform-name">{{ $t('steps.simulation.platform1') }}</span>
          ...
          <div class="platform-stats">
            <span class="stat">
              <span class="stat-label">{{ $t('steps.simulation.round') }}</span>
              <span class="stat-value mono">{{ runStatus.twitter_current_round || 0 }}<span class="stat-total">/{{ runStatus.total_rounds || maxRounds || '-' }}</span></span>
            </span>
            <span class="stat">
              <span class="stat-label">{{ $t('steps.simulation.elapsedTime') }}</span>
              <span class="stat-value mono">{{ twitterElapsedTime }}</span>
            </span>
            <span class="stat">
              <span class="stat-label">{{ $t('steps.simulation.acts') }}</span>
              <span class="stat-value mono">{{ runStatus.twitter_actions_count || 0 }}</span>
            </span>
          </div>
          <!-- Available actions tooltip -->
          <div class="actions-tooltip">
            <div class="tooltip-title">{{ $t('steps.simulation.availableActions') }}</div>
            <div class="tooltip-actions">
              <span class="tooltip-action">{{ $t('steps.simulation.actions.post') }}</span>
              <span class="tooltip-action">{{ $t('steps.simulation.actions.like') }}</span>
              <span class="tooltip-action">{{ $t('steps.simulation.actions.repost') }}</span>
              <span class="tooltip-action">{{ $t('steps.simulation.actions.quote') }}</span>
              <span class="tooltip-action">{{ $t('steps.simulation.actions.follow') }}</span>
              <span class="tooltip-action">{{ $t('steps.simulation.actions.idle') }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="platform-status reddit" :class="{ active: runStatus.reddit_running, completed: runStatus.reddit_completed }">
          <div class="platform-header">
            ...
            <span class="platform-name">{{ $t('steps.simulation.platform2') }}</span>
          ...
          <div class="actions-tooltip">
            ...
            <div class="tooltip-actions">
              <span class="tooltip-action">{{ $t('steps.simulation.actions.post') }}</span>
              <span class="tooltip-action">{{ $t('steps.simulation.actions.comment') }}</span>
              <span class="tooltip-action">{{ $t('steps.simulation.actions.like') }}</span>
              <span class="tooltip-action">{{ $t('steps.simulation.actions.dislike') }}</span>
              <span class="tooltip-action">{{ $t('steps.simulation.actions.search') }}</span>
              <span class="tooltip-action">{{ $t('steps.simulation.actions.trend') }}</span>
              <span class="tooltip-action">{{ $t('steps.simulation.actions.follow') }}</span>
              <span class="tooltip-action">{{ $t('steps.simulation.actions.mute') }}</span>
              <span class="tooltip-action">{{ $t('steps.simulation.actions.refresh') }}</span>
              <span class="tooltip-action">{{ $t('steps.simulation.actions.idle') }}</span>
            </div>
          </div>
        </div>
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? $t('steps.simulation.starting') : $t('steps.simulation.generateReport') }} 
    ...
    <div class="main-content-area" ref="scrollContainer">
      ...
      <div class="timeline-header" v-if="allActions.length > 0">
        <div class="timeline-stats">
          <span class="total-count">{{ $t('steps.simulation.totalEvents') }}: <span class="mono">{{ allActions.length }}</span></span>
      ...
      <div class="timeline-feed">
              ...
              <div class="card-body">
                ...
                <template v-if="action.action_type === 'REPOST'">
                  <div class="repost-info">
                    ...
                    <span class="repost-label">{{ $t('steps.simulation.actions.repostedFrom') }}{{ action.action_args?.original_author_name || 'User' }}</span>
                  </div>
                ...
                <template v-if="action.action_type === 'LIKE_POST'">
                  <div class="like-info">
                    ...
                    <span class="like-label">{{ $t('steps.simulation.actions.likedPost') }}{{ action.action_args?.post_author_name || 'User' }}{{ $t('steps.simulation.actions.postOf') }}</span>
                  </div>
                ...
                <template v-if="action.action_type === 'CREATE_COMMENT'">
                  ...
                  <div v-if="action.action_args?.post_id" class="comment-context">
                    ...
                    <span>{{ $t('steps.simulation.actions.replyTo') }}{{ action.action_args.post_id }}</span>
                  </div>
                </template>
                ...
                <template v-if="action.action_type === 'SEARCH_POSTS'">
                  <div class="search-info">
                    ...
                    <span class="search-label">{{ $t('steps.simulation.actions.searchQuery') }}</span>
                ...
                <template v-if="action.action_type === 'FOLLOW'">
                  <div class="follow-info">
                    ...
                    <span class="follow-label">{{ $t('steps.simulation.actions.followed') }}{{ action.action_args?.target_user || action.action_args?.user_id || 'User' }}</span>
                  </div>
                </template>
                ...
                <template v-if="action.action_type === 'DO_NOTHING'">
                  <div class="idle-info">
                    ...
                    <span class="idle-label">{{ $t('steps.simulation.actions.actionSkipped') }}</span>
                  </div>
                </template>
        ...
        <div v-if="allActions.length === 0" class="waiting-state">
          ...
          <span>{{ $t('steps.simulation.waiting') }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="system-logs">
      <div class="log-header">
        <span class="log-title">{{ $t('steps.simulation.monitor') }}</span>
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect round changes of each platform and output logs
      ...
      // Detect if simulation is complete (by runner_status or platform completion status)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Extra check: if backend hasn't updated runner_status yet but platforms have reported completion
      // Detect by checking twitter_completed and reddit_completed status
