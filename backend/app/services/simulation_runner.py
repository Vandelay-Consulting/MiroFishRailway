...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        if not os.path.exists(config_path):
            raise ValueError(
                f"Simulation config does not exist, please call /prepare API first"
            )
        ...
        try:
            ...
            # Create main log file to avoid stdout/stderr pipe buffer blocking process
            ...
            env = os.environ.copy()
            env["PYTHONUTF8"] = (
                "1"  # Python 3.7+ support, makes all open() use UTF-8 by default
            )
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Determines which platforms are enabled by checking if corresponding actions.jsonl file exists
    ...
    def _terminate_process(
        ...
        if IS_WINDOWS:
            ...
            try:
                ...
                # If timeout and still not ended, force send SIGKILL
                logger.warning(
    ...
    def _read_actions_from_file(
        ...
        Args:
            ...
            default_platform: Default platform (used when action record doesn't have platform field)
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean up simulation run logs (for force restart simulation)
        ...
        logger.info(
            f"Simulation log cleanup completed: {simulation_id}, deleted files: {cleaned_files}"
        )
    ...
    def register_cleanup(cls):
        ...
        Called when Flask app starts, ensures simulation processes are cleaned up when server shuts down
        """
        ...
        # If not in debug mode, there's no this env var, also need to register
        ...
        if is_debug_mode and not is_reloader_process:
            _cleanup_registered = (
                True  # Mark as registered, prevent child process from trying again
            )
            return
        ...
        try:
            ...
            logger.warning(
                "Cannot register signal handler (not in main thread), using atexit only"
            )
    ...
    def check_env_alive(cls, simulation_id: str) -> bool:
        ...
        Returns:
            True means environment is alive, False means environment is shut down
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            return False
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None: For dual-platform simulation, interview both platforms simultaneously, return merged result
        ...
        if not ipc_client.check_env_alive():
            raise ValueError(
                f"Simulation environment not running or has shut down, cannot execute Interview: {simulation_id}"
            )
    ...
    def close_simulation_env(
        ...
        Send close environment command to simulation, make it gracefully exit command-waiting mode
