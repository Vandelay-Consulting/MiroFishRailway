...
A. **Fallback types (must include, place last 2 in list)**:
   - `Person`: Fallback type for any natural person. Use when a person doesn't belong to other more specific types.
   - `Organization`: Fallback type for any organization. Use when an organization doesn't belong to more specific types.

B. **Specific types (8, design based on text content)**:
   - Design more specific types for major roles appearing in the text
   - Example: If text involves academic events: `Student`, `Professor`, `University`
   - Example: If text involves business events: `Company`, `CEO`, `Employee`

**Why fallback types are needed**:
- Text contains various people like "primary school teachers", "passerby A", "some netizen"
- If no specific type matches, they should be classified as `Person`
- Similarly, small organizations, temporary groups should be classified as `Organization`
...
- description must clearly explain the difference from fallback types
...
**Individual (Fallback)**:
- Person: Any natural person (use when not belonging to specific types above)
...
- Organization: Any organization (use when not belonging to specific types above)
