...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        ...
        Uses natural language description format to allow Zep to extract entities and relationships.
...
class ZepGraphMemoryUpdater:
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        self._skipped_count = 0  # Filtered and skipped activities (DO_NOTHING)
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        action_args contains complete context info (like original post content, username, etc.).
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                if attempt < self.MAX_RETRIES - 1:
                    ...
                    logger.error(
                        f"Batch send to Zep failed after {self.MAX_RETRIES} retries: {e}"
                    )
        ...
        with self._buffer_lock:
            for platform, buffer in self._platform_buffers.items():
                if buffer:
                    display_name = self._get_platform_display_name(platform)
                    logger.info(
                        f"Sending {len(buffer)} remaining {display_name} activities"
                    )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "skipped_count": self._skipped_count,  # Filtered and skipped activities (DO_NOTHING)
