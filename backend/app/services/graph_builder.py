...
class GraphBuilderService:
    ...
    def _build_graph_worker(
        ...
        try:
            ...
            # 6. Get graph info
            self.task_manager.update_task(
                task_id, progress=90, message="Fetching graph info..."
            )
    ...
    def _wait_for_episodes(
        ...
        if progress_callback:
            progress_callback(
                f"Starting to wait for {total_episodes} text chunks to process...", 0
            )
        ...
        if progress_callback:
            progress_callback(
                f"Processing complete: {completed_count}/{total_episodes}", 1.0
            )
