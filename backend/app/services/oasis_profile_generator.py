...
Converts entities from Zep knowledge graph to OASIS simulation platform Agent Profile format

Optimizations:
1. Use Zep retrieval to enrich node information
2. Optimized prompts to generate detailed personas
...
class OasisProfileGenerator:
    ...
    Converts entities from Zep knowledge graph to Agent Profiles for OASIS simulation

    Optimized features:
    1. Use Zep graph retrieval to get richer context
    2. Generate detailed personas (including basic info, career, personality, social media behavior, etc.)
    3. Distinguish between individual entities and abstract group entities
    """
    ...
    # Group/Organization entity types (need to generate representative personas)
    GROUP_ENTITY_TYPES = [
        "university",
        "governmentagency",
        "organization",
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Zep hybrid search completed: {entity_name}, retrieved {len(results['facts'])} facts, {len(results['node_summaries'])} related nodes"
            )
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        # 3. Add detailed info of related nodes
    ...
    def _generate_profile_with_llm(
        ...
        - Group/Organization entity: Generate representative account setting
        """
        ...
        # Try multiple times to generate, until success or max retries reached
        max_attempts = 3
        last_error = None
        ...
        for attempt in range(max_attempts):
            try:
                response = self.client.chat.completions.create(
                    ...
                    # Don't set max_tokens, let LM generate freely
                )
                ...
                if finish_reason == "length":
                    logger.warning(
                        f"LLM output truncated (attempt {attempt + 1}), attempting to fix..."
                    )
    ...
    def _fix_truncated_json(self, content: str) -> str:
        ...
        # Simple check: if last char is not quote or closing brace, string may be truncated
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user persona generation expert. Generate detailed, realistic personas for social simulation, accurately reflecting real-world situations. Must return valid JSON format, all string values must not contain unescaped newlines. Use English."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the entity, accurately reflecting real-world situations.
...
2. persona: Detailed persona description (~2000 words plain text), must include:
   ...
   - Personal Background (significant experiences, connection to events, social relationships)
   ...
   - Stances & Views (attitudes toward topics, what might trigger/upset them)
   - Unique Characteristics (catchphrases, special experiences, hobbies)
...
- Use English for ALL content (except gender which must be "male"/"female")
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account persona for the organization/group entity, accurately reflecting real-world situations.
...
2. persona: Detailed account persona description (~2000 words plain text), must include:
   ...
   - Organization Memory (important part of organization persona - describe this organization's connection to events and their actions/reactions)
3. age: Fixed value 30 (virtual age for organization accounts)
4. gender: Fixed value "other" (organization accounts use "other" to indicate non-personal)
...
- Use English for ALL content (except gender which must be "other")
- age must be integer 30, gender must be string "other"
- Organization account posts should align with their identity positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            realtime_output_path: Real-time output file path (if provided, write each time one is generated)
        ...
        print(f"\n{'=' * 60}")
        print(
            f"Starting Agent persona generation - {total} entities total, parallel count: {parallel_count}"
        )
        print(f"{'=' * 60}\n")
        ...
        print(f"\n{'=' * 60}")
        print(
            f"Persona generation complete! Generated {len([p for p in profiles if p])} Agents"
        )
        print(f"{'=' * 60}\n")
