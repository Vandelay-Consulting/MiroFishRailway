...
2. First plan the outline structure, then generate sections sequentially
3. Each section uses ReACT multi-round thinking and reflection mode
4. Support user dialogue with autonomous tool invocation for retrieval
"""
...
class ReportLogger:
    ...
    Generates agent_log.jsonl file in the report folder to record every detailed action.
...
class ReportConsoleLogger:
    ...
    def __del__(self):
        """Ensure file handler is closed on destructor"""
        self.close()
...
This tool is used to get the complete picture of simulation results, especially suitable for understanding the event evolution process. It will:
...
PLAN_SYSTEM_PROMPT = """\
You are an expert in writing "Future Prediction Reports" with a "God's-eye view" of the simulated world - you can see every Agent's behavior, statements, and interactions in the simulation.

【Core Concept】
We have built a simulated world and injected specific "simulation requirements" as variables. The evolution of the simulated world is a prediction of what may happen in the future. You are observing not "experimental data" but a "preview of the future".
...
- Section structure is designed autonomously based on prediction results

Please output the report outline in JSON format as follows:
{
    "title": "Report Title (in English)",
    "summary": "Report summary - one sentence summarizing the core prediction findings (in English)",
    "sections": [
        {
...
【IMPORTANT】The entire report MUST be in English! All titles, summaries, and content must be written in English.
...
【Reminder】Report section quantity: minimum 2, maximum 5, content should be concise and focused on core prediction findings."""
...
The simulated world is a preview of the future. We have injected specific conditions (simulation requirements) into the simulated world.
...
1. 【Must Use Tools to Observe Simulated World】
   ...
   - Each section must call tools at least 3 times (maximum 5 times) to observe the simulated world that represents the future

2. 【Must Quote Agent Original Behavior】
   ...
   - Use quote format in the report to display these predictions, for example:
     > "Certain groups expressed: original content..."
...
3. 【Language - Report must be in English】
   ...
   - If the simulation content or tool results are in Chinese, translate ALL content to English
   - When quoting tool results or agent responses, translate Chinese content to fluent English
   - Maintain original meaning while ensuring natural English expression
   - This applies to both main text and quoted content (> format)

4. 【Faithfully Present Prediction Results】
   - Report content must reflect simulation results in the simulated world that represent the future
   - Do not add information that does not exist in the simulation
   - If information is insufficient in certain aspects, state this honestly
...
【Correct Example】
```
This section analyzes the public opinion dissemination situation。Through in-depth analysis of simulation data,we found...

**Initial Outbreak Phase**

Weibo serves as the first scene of public opinion,and bears the core function of initial information release：
...
- Prohibited from including both tool calls and Final Answer in one response
...
4. 【Quote Format Specification - Must be separate paragraphs】
   Quotes must be independent paragraphs,with blank lines before and after,cannot be mixed in paragraphs：
...
- If information is sufficient: output output the section content starting with(must quote the original text above)
...
REACT_TOOL_LIMIT_MSG = (
    ...
    'Please immediately based on obtained information,output the section content starting with "Final Answer:".'
)

REACT_UNUSED_TOOLS_HINT = "\n💡 You have not used yet: {unused_list},try different tools for multi-angle information"

REACT_FORCE_FINAL_MSG = (
    "Tool call limit reached, please directly output Final Answer: and generate section content."
)
...
3. Only when the report content is insufficient to answer,then call tools to retrieve more data
...
class ReportAgent:
    ...
    def __init__(
        ...
        logger.info(
            f"ReportAgent initialization completed: graph_id={graph_id}, simulation_id={simulation_id}"
        )
    ...
    def _execute_tool(
        ...
        try:
            if tool_name == "insight_forge":
                ...
                # Interview agents - call real OASIS interview API to get agent responses (dual platform)
    ...
    # Valid tool names set, used for validation in bare JSON fallback parsing
    VALID_TOOL_NAMES = {
        "insight_forge",
        "panorama_search",
        "quick_search",
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Only try when format 1 does not match, to avoid false matching JSON in main text
    ...
    def plan_outline(
        ...
        try:
            ...
            return ReportOutline(
                ...
                sections=[
                    ReportSection(title="Predicted Scenario and Key Findings"),
    ...
    def _generate_section_react(
        ...
        for iteration in range(max_iterations):
            ...
            if has_tool_calls and has_final_answer:
                conflict_retries += 1
                logger.warning(
                    f"section {section.title} round: "
                    f"LLM output both tool calls and Final Answer(#{conflict_retries} conflicts)"
                )

                if conflict_retries <= 2:
                    # First two times: discard this response and ask LLM to respond again
            ...
            if has_final_answer:
                ...
                logger.info(
                    f"section {section.title} generation completed (tool call: {tool_calls_count} times)"
                )
            ...
            if has_tool_calls:
                ...
                if len(tool_calls) > 1:
                    logger.info(
                        f"LLM tried to call {len(tool_calls)} tools, only executing first: {call['name']}"
                    )
    ...
    def generate_report(
        ...
        try:
            ...
            ReportManager.update_progress(
                ...
                "planning",
                5,
                "Started planning report outline...",
                completed_sections=[],
            )
            ...
            # Stage 2: Generate sections sequentially (save each section)
            ...
            try:
                ...
                pass  # Ignoring save failure error
