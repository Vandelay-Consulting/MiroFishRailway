...
Encapsulates graph search, node reading, edge query tools for Report Agent usage
...
class AgentInterview:
    """Individual Agent interview result"""
...
class ZepToolsService:
    ...
    def search_graph(
        ...
        If Zep Cloud search API is unavailable, fall back to local keyword matching。
    ...
    def _local_search(
        ...
        def match_score(text: str) -> int:
            """Calculate text-query matching score"""
            if not text:
                return 0
    ...
    def get_node_detail(self, node_uuid: str) -> Optional[NodeInfo]:
        ...
        try:
            ...
            logger.error(f"Getting node detailsfailed: {str(e)}")
            return None
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        Get all graph edges, then filter out edges related to the specified node
    ...
    def get_entity_summary(self, graph_id: str, entity_name: str) -> Dict[str, Any]:
        ...
        Search for all information related to the entity and generate summary
    ...
    def insight_forge(
        ...
        # Get details of all related entities (unlimited, complete output)
        entity_insights = []
...
Return the sub-questions list in JSON format."""
    ...
    def panorama_search(
        ...
        logger.info(
            f"PanoramaSearch completed: {result.active_count}valid, {result.historical_count}historical"
        )
        return result
    ...
    def interview_agents(
        ...
        Args:
            ...
            interview_requirement: Interview requirement description (unstructured, e.g., "understand student opinions on the event")
        ...
        logger.info(
            f"InterviewAgents in-depth interview (real API): {interview_requirement[:50]}..."
        )
        ...
        if not profiles:
            logger.warning(f"Simulation not found {simulation_id} profile file")
        ...
        INTERVIEW_PROMPT_PREFIX = (
            ...
            "4. Answer each question by number, start each answer with 「Question X:」 (X is the question number)\n"
            "5. Separate each answer with a blank line\n"
            "6. Provide substantive answers, at least 2-3 sentences per question\n\n"
        )
        optimized_prompt = f"{INTERVIEW_PROMPT_PREFIX}{combined_prompt}"

        # Step 4: Call real interview API (no platform specified, dual platform simultaneous interview by default)
        try:
    ...
    def _load_agent_profiles(self, simulation_id: str) -> List[Dict[str, Any]]:
        ...
        if os.path.exists(reddit_profile_path):
            try:
                ...
                logger.info(
                    f"Loaded from reddit_profiles.json {len(profiles)} profiles"
                )
                return profiles
            except Exception as e:
        ...
        if os.path.exists(twitter_profile_path):
            try:
                ...
                logger.info(
                    f"Loaded from twitter_profiles.csv {len(profiles)} profiles"
                )
                return profiles
            except Exception as e:
...
Please generate 3-5 interview questions."""
        ...
        try:
            ...
            return response.get(
                "questions", [f"What is your opinion on {interview_requirement}?"]
            )
