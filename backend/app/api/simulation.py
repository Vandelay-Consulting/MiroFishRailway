...
def get_graph_entities(graph_id: str):
    ...
    try:
        ...
        logger.info(
            f"Retrieving graph entities: graph_id={graph_id}, entity_types={entity_types}, enrich={enrich}"
        )
...
def create_simulation():
    ...
    Note: max_rounds and other parameters are intelligently generated by LLM, no need to set manually

    Request (JSON):
        {
            "project_id": "proj_xxxx",      // required
            "graph_id": "mirofish_xxxx",    // optional, will get from project if not provided
        ...
        if status in prepared_statuses and config_generated:
            ...
            if status == "preparing":
                try:
                    ...
                    logger.info(
                        f"Auto-updated simulation status: {simulation_id} preparing -> ready"
                    )
                    status = "ready"
                except Exception as e:
...
def prepare_simulation():
    ...
    try:
        ...
        logger.info(
            f"Processing /prepare request: simulation_id={simulation_id}, force_regenerate={force_regenerate}"
        )
        ...
        if not force_regenerate:
            ...
            if is_prepared:
                logger.info(
                    f"Simulation {simulation_id} already prepared, skipping regeneration"
                )
                return jsonify(
                    {
                        "success": True,
                ...
                logger.info(
                    f"Simulation {simulation_id} not prepared, will start preparation task"
                )
...
def get_prepare_status():
    ...
    try:
        ...
        if not task:
            # Task does not exist, but if simulation_id exists, check if preparation is complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify(
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    return the latest if multiple exist (sorted by created_at)
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Get simulation Agent Profiles in real-time (used for viewing progress during generation)
...
def get_simulation_config_realtime(simulation_id: str):
    """
    Get simulation configuration in real-time (used for viewing progress during generation)
...
def generate_profiles():
    ...
    try:
        ...
        if filtered.filtered_count == 0:
            return jsonify(
                {"success": False, "error": "No entities matching criteria found"}
            ), 400
...
def start_simulation():
    ...
    About force parameter:
        ...
        - Cleaning includes: run_state.json, actions.jsonl, simulation.log, etc.
    ...
    About enable_graph_memory_update:
        - When enabled, all Agent activities (posts, comments, likes, etc.) during simulation will be updated to Zep graph in real-time
        - This allows the graph to "remember" the simulation process, for subsequent analysis or AI conversations
        - Requires valid graph_id for associated project
    ...
    try:
        ...
        if enable_graph_memory_update:
            ...
            logger.info(
                f"Enabled graph memory update: simulation_id={simulation_id}, graph_id={graph_id}"
            )
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # recent_actions only shows content from both platforms in latest round
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be running (enters wait command mode after simulation loop completes)
    ...
    try:
        ...
        if platform and platform not in ("twitter", "reddit"):
            return jsonify(
                {"success": False, "error": "Platform parameter must be 'twitter' or 'reddit'"}
            ), 400
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify(
                {
                    "success": False,
                    "error": "Simulation environment is not running or has been closed. Please ensure simulation is completed and in wait command mode.",
                }
            ), 400
        ...
        return jsonify(
            {"success": False, "error": f"Interview response timeout: {str(e)}"}
        ), 504
...
def interview_agents_batch():
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // Required, interview list
                ...
                {
                    "agent_id": 1,
                    "prompt": "What do you think about B?"  // if platform not specified, use default value
                }
            ],
    ...
    try:
        ...
        if not interviews or not isinstance(interviews, list):
            return jsonify(
                {"success": False, "error": "Please provide interviews (interview list)"}
            ), 400
        ...
        for i, interview in enumerate(interviews):
            if "agent_id" not in interview:
                return jsonify(
                    {"success": False, "error": f"Interview list item {i + 1} missing agent_id"}
                ), 400
            if "prompt" not in interview:
                return jsonify(
                    {"success": False, "error": f"Interview list item {i + 1} missing prompt"}
                ), 400
        ...
        return jsonify(
            {"success": False, "error": f"Batch Interview response timeout: {str(e)}"}
        ), 504
...
def interview_all_agents():
    ...
    try:
        ...
        if not prompt:
            return jsonify(
                {"success": False, "error": "Please provide prompt (interview question)"}
            ), 400
        ...
        return jsonify(
            {"success": False, "error": f"Global Interview response timeout: {str(e)}"}
        ), 504
...
def close_simulation_env():
    ...
    Note: This is different from /stop interface, /stop will forcibly terminate the process,
    while this interface allows simulation to gracefully close environment and exit.
